...
class NuScenesMatchDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.nuscenes_infos_10sweeps_train = config["nuscenes_infos_10sweeps_train"]
        self.nuscenes_infos_10sweeps_val = config["nuscenes_infos_10sweeps_val"]
        ...
        # self.list_keyframes = self.list_keyframes[:200]

        with open(self.nuscenes_infos_10sweeps_train, 'rb') as f:
    ...
    def map_pointcloud_to_image(self, point_merged, data, lidar_name, min_dist: float = 1.0,  multi_sweeps=True):
        ...
        for i, camera_name in enumerate(camera_list):
            ...
            #     cv2.circle(im, (int(matching_pixels[i][1]), int(matching_pixels[i][0])), 1, (255, 255, 0), -1)
            # if multi_sweeps:
            #     cv2.imwrite("image_and_points/"+camera_name + 'multi_sweeps.png', im)
            # else:
            #     cv2.imwrite("image_and_points/"+camera_name + '.png', im)
    ...
    def __getitem__(self, idx):
        ...
        # print(len(self.sweeps_infos))
        # print(self.sweeps_infos)
        ...
        for sweep_id, sweep_points in enumerate(sweep_points_list):
            ...
            # images_print = images.permute(0, 2, 3, 1).numpy() * 255
            # cv2.imwrite("visual_image/before"+str(sweep_id)+".png", images_print[0])
            # cv2.imwrite("visual_image/before2.png", images_print[1])
            # cv2.imwrite("visual_image/before3.png", images_print[2])
            # cv2.imwrite("visual_image/before4.png", images_print[3])
            ...
            # images_print = images.permute(0, 2, 3, 1).numpy() * 255
            # cv2.imwrite("visual_image/after"+str(sweep_id)+".png", images_print[0])
            # cv2.imwrite("visual_image/after2.png", images_print[1])
            # cv2.imwrite("visual_image/after3.png", images_print[2])
            # cv2.imwrite("visual_image/after4.png", images_print[3])
            ...
            sweepIds_group.append(sweep_id * torch.ones(discrete_coords.shape[0]))
            sweep_pairing_group.append(sweep_id * torch.ones(pairing_images.shape[0]))
