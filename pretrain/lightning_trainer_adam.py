...
class LightningPretrain(pl.LightningModule):
    def __init__(self, model_points, model_images, model_fusion, config):
        ...
        self.embedding_linear = torch.nn.Linear(256, 512)
        self.image_feats_linear = torch.nn.Linear(256, 512)  # Linearly expand image features to 512
    ...
    def configure_optimizers(self): 
        optimizer = optim.Adam(
            ...
            lr=self._config.get("lr", 1e-4),  # 建议 lr 设置为 1e-4
            weight_decay=self._config.get("weight_decay", 0.0001),
        )
        # 使用学习率调度器（可选）
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        # Set 3D model to train mode
        self.model_points.train()
        # extract input data
        sinput_C = batch["sinput_C"] # sinput_C shape: torch.Size([25820, 4])
        #print(f"sinput_C shape: {sinput_C.shape}")
        
        sinput_F = batch["sinput_F"] # sinput_F shape: torch.Size([25642, 3])
        #print(f"sinput_F shape: {sinput_F.shape}")
        

        if self._config['dataset'] == "nuscenes":
            sweepIds = batch["sweepIds"]

        # Update coordinates for multiple sweeps
        ...
        # Prepare SparseTensor input for the 3D backbone
        ...
        # Forward pass through 3D model
        output_points = self.model_points(sparse_input)
        # Forward pass through LISA model for multi-modal input
        ...
        if not self.saved and self.epoch == 10:
            self.save()
            self.saved = True
    ...
    def scannet_loss(self, batch, output_points, output_images):
        ...
        # Extract features from 3D model output
        ...
        # Extract predictions and features from LISA output
        ...
        #print(f"image_feats shape: {image_feats.shape}, image_feats dtype:{image_feats.dtype}")
        ...
        text_embeddings = self.embedding_linear(pred_embeddings.float()) 
        ...
        #     if rd > 5: 
        #         q_pred = k_logists.argmax(dim=1)
        # 可切换训练策略
        if self.epoch >= 10:
            rd = random.randint(1, 10)
            if rd > 5:
                q_pred = (k_logists >= 0).float()  # 将 logits 转换为二值预测

        # Cross-entropy loss for semantic consistency
        q_pred = q_pred.float().unsqueeze(1).expand(-1, k_logists.size(1))
        # q_pred = q_pred.float()
        # if k_logists.dim() > 1:
        #     k_logists = k_logists.squeeze()
        loss_semantic = self.BCE(k_logists, q_pred)
        ...
        image_feats_expanded = self.image_feats_linear(image_feats.float())  # Expand to 512 dimensions

        # Update view to dynamically adapt to batch size and spatial dimensions
        ...
        self.log("loss_semantic", loss_semantic, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        self.log("loss_spatial", loss_spatial, on_step=True, on_epoch=True, prog_bar=True, logger=True)
