    ...
    def forward(self, x, text_embeddings=None):
        ...
        if self.config['mode'] == 'pretrain':
            ...
            if text_embeddings is None:
                raise ValueError("text_embeddings must be provided in 'source_free' mode.")
            out = F.conv1d(feat.F.unsqueeze(-1), text_embeddings[:, :, None]).squeeze()
            #out = F.conv1d(feat.F.unsqueeze(-1), self.text_embeddings[:, :, None]).squeeze()
            return out
        
        elif self.config['mode'] == 'reasonseg':
            out = self.final(feats)
            return out.F
