...
class LightningDownstream(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, lb in enumerate(batch["len_batch"]):
            ...
            labels.append(batch["sem_labels"][i])
            offset += lb
        ...
        self.validation_outputs.append((loss, c_matrix))
    ...
    def on_validation_epoch_end(self):
        ...
        self.train_losses = []
        self.val_losses = []
        if m_IoU > self.max_miou:
            self.max_miou = m_IoU
            self.log("Maxm_IoU", self.max_miou, prog_bar=True, logger=True, sync_dist=False)
