...
def zeroshot_classifier(model_name, classnames, templates):
    ...
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            zeroshot_weights_all.append(class_embeddings)
        zeroshot_weights = torch.stack(zeroshot_weights, dim=1).cuda()
        zeroshot_weights_all = torch.stack(zeroshot_weights_all, dim=1).cuda()

    return zeroshot_weights, zeroshot_weights_all
...
if __name__ == '__main__':
    ...
    zeroshot_weights, zeroshot_weights_all = zeroshot_classifier(name_mapping[args.model], classes, prompt_templates)
    zeroshot_weights = zeroshot_weights.permute(1, 0).float()
    # zeroshot_weights_all = zeroshot_weights_all.permute(1, 0, 2).float()
    print(zeroshot_weights.shape)
    print(zeroshot_weights_all.shape)

    prefix = f'{all_set_name}_{args.model}'
    prefix_all = f'{all_set_name}_{args.model}' + '_all'
    if args.no_prompt_eng:
        prefix += '_npe'

    # torch.save(zeroshot_weights, f'{prefix}_clip_text.pth')
    torch.save(zeroshot_weights_all, f'{prefix_all}_clip_text.pth')
